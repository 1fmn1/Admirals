...
Public Class ShopControl
    ...
    Private Async Sub Button_Click_Confirm(sender As Object, e As RoutedEventArgs)
        ...
        Logger.LogWrite($"Attempting to buy [{tabIt.Name}] for [{price.Text}] CUAG  on starport [{gdata}]...")
        Do While WC.IsBusy
            Await Task.Delay(200)
        Loop
        s = Await WC.DownloadStringTaskAsync($"http://galaxy.xjedi.com/srv/conn.php?act=ship_buy&starport_id={gdata.id}&ship_spec_id={tabIt.Header}&pay_sum={price.Text}")
        ...
        If gwpack.code <> "OK" Then
            ...
            If msg = MsgBoxResult.Ok Then
                Do While WC.IsBusy
                    Await Task.Delay(200)
                Loop
                s = Await WC.DownloadStringTaskAsync($"http://galaxy.xjedi.com/srv/conn.php?act=ship_buy&starport_id={gdata.id}&ship_spec_id={tabIt.Header}&pay_sum={gwpack.cost}")
            Else
